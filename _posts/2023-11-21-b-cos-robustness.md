---
layout: post
title: "Probing B-cos Networks' Robustness"
...
To evaluate the adversarial robustness of the trained B-cos model, I utilized the Robustness library [[4](#ref4)]. This library facilitates a comparison of the obtained results with those of a naturally and adversarially trained ResNet-50 model provided by the library's authors ([link to pretrained models](https://github.com/madrylab/robustness#pretrained-models){:target="_blank"}). In addition, this library makes it easy to attack a custom loss function, a necessity in this instance due to the Binary Cross-Entropy loss function employed during the B-cos model training process.
...
It is important to note that the results presented herein should be interpreted with caution due to the disparity in data augmentation pipelines employed for the B-cos model and the provided ResNet-50 models.
...
Considering that this is not an exact replication of the results reported in the paper, a slight improvement in the robustness of the B-cos model could potentially be achieved if a more precise reproduction was attained.

The adversarially trained models showed consistently superior robustness. Nevertheless, the B-cos model displayed modest inherent robustness, particularly against {::nomarkdown}\(\ell_2\){:/} attacks, unlike naturally trained models. This finding provides evidence that inherently interpretable models represent a promising direction for both interpretability and robustness.
